...
group.add_argument('--output', default='outputs', type=str, metavar='PATH',
...
def main():
    ...
    if utils.is_primary(args):
        ...
        _logger.info(model)
        _logger.info(parameter_count_table(model))
        # flops = FlopCountAnalysis(model, (torch.rand(1, 4, 224, 224),))
        # _logger.info(f'FLOPs: ({flops.total()})')
    ...
    # model_pth = torch.load('/defaultShare/archive/yinbowen/VIT/RetD/DFormer_new-main/checkpoints/pretrained/RMT-S.pth')
    
    # state_dict = OrderedDict()
    # for k, v in model_pth['model'].items():
...
def validate(
    ...
    # wandb.log({'loss': losses_m.avg, 'top1': top1_m.avg,'top5':top5_m.avg})
